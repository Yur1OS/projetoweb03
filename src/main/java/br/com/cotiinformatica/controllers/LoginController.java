...
public class LoginController {
	...
	public ResponseEntity<String> post(@RequestBody PostLoginDTO dto) {
		try {
			...
			//pesquisar o usuário no BD através do email e da senha
			UsuarioRepository usuarioRepository = new UsuarioRepository();
			Usuario usuario = usuarioRepository.findByEmailAndSenha(dto.getEmail(), MD5Helper.encrypt(dto.getSenha()));
			
			//VRF se o usuário foi encontrado
			if(usuario != null) {
				return ResponseEntity.status(HttpStatus.OK).body(TokenSecurity.generateToken(usuario.getEmail()));
			}
			else {
				return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Acesso negado. Usuário inválido !!");
			}
		}
		catch (Exception e) {
